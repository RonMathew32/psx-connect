...
function createFixClient(options) {
    ...
    const handleLogon = (message) => {
        ...
        if (message[constants_1.FieldTag.RESET_SEQ_NUM_FLAG] === 'Y') {
            ...
            logger_1.default.info(`Reset sequence flag is Y, setting our next sequence number to ${msgSeqNum}`);
        }
        else {
        ...
        requestSecurityList: () => {
            ...
            if (socket) {
                ...
                setTimeout(() => {
                    ...
                    msgSeqNum++; // Increment sequence number
                    logger_1.default.info(`[SECURITY_LIST] Index security list request sent successfully. Next sequence: ${msgSeqNum}`);
