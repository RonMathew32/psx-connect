...
function parseFixMessage(message) {
    try {
        ...
        if (result[constants_1.FieldTag.NO_MD_ENTRIES]) {
            const noMDEntries = parseInt(result[constants_1.FieldTag.NO_MD_ENTRIES], 10);
            ...
            for (let i = 1; i <= noMDEntries; i++) {
                ...
                if (mdEntryType) {
                    ...
                    result[`${entryPrefix}:${constants_1.FieldTag.MD_ENTRY_TYPE}`] = entryTypeVal;
                    if (mdEntryPx) {
                        const pxVal = mdEntryPx.substring(mdEntryPx.indexOf('=') + 1);
                        result[`${entryPrefix}:${constants_1.FieldTag.MD_ENTRY_PX}`] = pxVal;
                    }
                    if (mdEntrySize) {
                        const sizeVal = mdEntrySize.substring(mdEntrySize.indexOf('=') + 1);
                        result[`${entryPrefix}:${constants_1.FieldTag.MD_ENTRY_SIZE}`] = sizeVal;
                    }
                    if (mdPriceLevel) {
                        const levelVal = mdPriceLevel.substring(mdPriceLevel.indexOf('=') + 1);
                        result[`${entryPrefix}:${constants_1.FieldTag.MD_PRICE_LEVEL}`] = levelVal;
                    }
                    ...
                    if (noOrders) {
                        ...
                        result[`${entryPrefix}:${constants_1.FieldTag.NO_ORDERS}`] = noOrdersVal;
                        ...
                        for (let j = 1; j <= numOrders; j++) {
                            ...
                            if (orderQty) {
                                ...
                                result[`${orderPrefix}:${constants_1.FieldTag.ORDER_QTY}`] = qtyVal;
                            }
                            if (orderId) {
                                const idVal = orderId.substring(orderId.indexOf('=') + 1);
                                result[`${orderPrefix}:${constants_1.FieldTag.ORDER_ID}`] = idVal;
                            }
                        }
                    }
                    ...
                    if (numberOfOrders) {
                        ...
                        result[`${entryPrefix}:${constants_1.FieldTag.NUMBER_OF_ORDERS}`] = numVal;
                    }
                }
            }
        }
