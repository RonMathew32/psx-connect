...
class SequenceManager {
    constructor(initialSeq) {
        ...
        this.securityListSeqNum = initialSeq?.securityList ?? 2; // PSX-specific default
        this.tradingStatusSeqNum = initialSeq?.tradingStatus ?? 2; // PSX-specific default
    ...
    processLogon(serverSeqNum, resetFlag) {
        ...
        if (resetFlag) {
            ...
            logger_1.logger.info(`[SEQUENCE] Reset sequence flag is Y, setting sequence numbers: Main=${this.mainSeqNum}, SecurityList=${this.securityListSeqNum}, TradingStatus=${this.tradingStatusSeqNum}, MarketData=${this.marketDataSeqNum}`);
        }
        else {
     ...
     */
    resetAll() {
        logger_1.logger.info('[SEQUENCE] Resetting all sequence numbers to initial values');
        this.mainSeqNum = 1;
        this.serverSeqNum = 1;
    ...
    resetSecurityListSequence() {
        logger_1.logger.info('[SEQUENCE] Resetting security list sequence number to 2');
        this.securityListSeqNum = 2;
        // Also update main sequence if needed
        if (this.mainSeqNum < 2) {
            logger_1.logger.info(`[SEQUENCE] Also updating main sequence to 2 to maintain alignment`);
            this.mainSeqNum = 2;
        }
    }
    /**
