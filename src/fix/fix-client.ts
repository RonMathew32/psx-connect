...
export function createFixClient(options: FixClientOptions) {
  ...
  const handleLogon = (message: ParsedFixMessage, sequenceManager: SequenceManager, emitter: EventEmitter): void => {
    ...
    if (resetFlag) {
      ...
      logger.info(`[SESSION:LOGON] Reset sequence flag is Y, resetting all sequence numbers`);
      sequenceManager.resetAll();
      
      // After reset, force the main sequence number to 2 (next message after logon)
      sequenceManager.forceReset(2);
      logger.info(`[SESSION:LOGON] Sequence numbers after reset: ${JSON.stringify(sequenceManager.getAll())}`);
    } else {
      // Otherwise, set our next sequence to match what the server expects
      logger.info(`[SESSION:LOGON] Using server's sequence number to align our sequence numbers`);
      
      // Update using server's sequence number
      sequenceManager.forceReset(serverSeqNum + 1);
      logger.info(`[SESSION:LOGON] Sequence numbers after alignment: ${JSON.stringify(sequenceManager.getAll())}`);
    }
  ...
  const sendTradingSessionStatusRequest = (): string | null => {
    try {
      ...
      logger.info(`[SEQUENCE] Using FIXED security list sequence number: ${securityListSeqNum} (must be 2)`);
      ...
      socket.write(rawMessage);
      logger.info(`[TRADING_STATUS:REQUEST] Sent request for REG market with ID: ${requestId} | Using sequence: ${securityListSeqNum} (must be 2)`);
      return requestId;
    } catch (error) {
  ...
  const sendSecurityListRequestForEquity = (): string | null => {
    try {
      ...
      if (socket) {
        ...
        logger.info(`[SECURITY_LIST:EQUITY] Product: EQUITY | Market: REG | Using sequence: ${securityListSeqNum} (must be 2)`);
        return requestId;
      } else {
  ...
  const sendSecurityListRequestForIndex = (): string | null => {
    try {
      ...
      if (socket) {
        ...
        logger.info(`[SECURITY_LIST:INDEX] Product: INDEX | Market: REG | Using sequence: ${securityListSeqNum} (must be 2)`);
        return requestId;
      } else {
